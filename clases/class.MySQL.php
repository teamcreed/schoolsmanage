...
class MySQL {
    ...
    private $MySQLi;            // MODIFICANDO A MYSQLI
    ...
    public function executeSQL($query,$sel=0){
    ...
    public function limpiar(&$value){
        $value = addslashes($this->MySQLi->real_escape_string($value));
    }
    ...
    public function select($from, $where='', $orderBy='', $limit='', $like=false, $operand='AND',$cols='*',$distinto='',$having=''){
        // Catch Exceptions
        if(trim($from) == ''){
            return false;
        }
...
//        if($from=='geozonas')
            //echo $query;
        return $this->executeSQL($query,1);

    }

    // selects a record in the database based on WHERE
...
//        }
    ...
    public function countRows($from, $where=''){
        $result = $this->select($from, $where, '', '', false, 'AND','count(*) AS CONTEO');
